...
Remember, the user interface is available at `http://{docker-machine-ip}:8080`. To alter the currently played song, change the URL query params inside the browser window, e.g., `http://{docker-machine-ip}/?title={songtitle}&artist={artist name}`.
