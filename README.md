...
The application container can be based on `alpine`.
As a second step just install Java via `apk add -U openjdk8-jre`.
...
Make sure to set appropriate dependencies and
links to allow an automatic bootstrapping of the service composition via `docker-compose up`.
...
As an additional improvement, define separate named networks inside the Compose file that isolate at least the frontend from the backend services or even all of the services from each other.
